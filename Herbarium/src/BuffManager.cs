...
namespace BuffStuff
{
    public class BuffManager
    {
        ...
        {
            if (!isInitialized) throw new Exception("BuffManager.RegisterBuff: must call BuffManager.Initialize() first!");
            if (buffTypes.ContainsKey(buffTypeId)) throw new Exception($"BuffManager.RegisterBuff: buffId already registered: {buffTypeId}");
        ...
        private static SerializedBuff serializeBuff(Buff buff)
        {
            ...
            return new SerializedBuff()
            {
                ...
                timeRemainingInDays = buff.ExpireTimestampInDays - Now,
        ...
        private static Buff deserializeBuff(SerializedBuff serializedBuff, Entity entity)
        {
            ...
            buff.entity = entity;
            buff.ExpireTimestampInDays = Now + serializedBuff.timeRemainingInDays;
        ...
        public static void Initialize(ICoreAPI api, ModSystem mod)
        {
            Api = api;
            if (api.Side == EnumAppSide.Server)
            {
                ...
                sapi.Event.GameWorldSave += () =>
                {
                    ...
                    { // add active buffs too!
                            activeAndInactiveBuffsByPlayerUid[(entityActiveBuffsByBuffIdPair.Key as EntityPlayer)?.PlayerUID] = entityActiveBuffsByBuffIdPair.Value.Select(pair => serializeBuff(pair.Value)).ToList();
                    }
                ...
                {
                    if (inactiveBuffsByPlayerUid.TryGetValue(serverPlayer.Entity.PlayerUID, out var inactiveBuffs))
                    {
                        ...
                        inactiveBuffsByPlayerUid.Remove(serverPlayer.Entity.PlayerUID);
                ...
                sapi.Event.PlayerLeave += (serverPlayer) =>
                {
                    if (activeBuffsByEntityAndBuffId.TryGetValue(serverPlayer.Entity, out var activeBuffsByBuffId))
                    {
                        ...
                        inactiveBuffsByPlayerUid[serverPlayer.Entity.PlayerUID] = activeBuffsByBuffId.Select(pair => serializeBuff(pair.Value)).ToList();
        ...
        {
            if (activeBuffsByEntityAndBuffId.TryGetValue(entity, out var activeBuffs) && activeBuffs.TryGetValue(buffId, out var buff)) return buff;
            return null;
        }
        ...
        {
            if (!isInitialized) throw new Exception("BuffManager.RegisterBuff: must call BuffManager.Initialize() first");
            if (!buffTypeToIds.ContainsKey(buff.GetType())) throw new Exception($"BuffManager.RegisterBuff: must call BuffManager.RegisterBuffType() first for buff type {buff.GetType().FullName}");
            ...
            if (activeBuffsByEntityAndBuffId[entity].TryGetValue(buffId, out var oldBuff)) buff.OnStack(oldBuff);
            else buff.OnStart();
        ...
        { // n.b. does not call any Buff event callbacks!
            if (activeBuffsByEntityAndBuffId.TryGetValue(entity, out var activeBuffs))
            {
                ...
                {
                    if (activeBuffs.Count == 0) activeBuffsByEntityAndBuffId.Remove(entity); // cleanup dictionary pair with now-empty list!
                    return true;
                }
            }
            return false;
